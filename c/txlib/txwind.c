...
static BYTE  StaticVbuf[4096];                  // possible VIO bug avoider
       BYTE *txVbuf = StaticVbuf;               // scrollbuf upto 2K  cells
...
)
{
...
#if !defined (UNIX)
   ULONG               columns = TxScreenCols();
   ULONG               option  = 0;             // default AUTO
#endif
   ...
   ENTER();
      ...
      #if defined (WIN32)
         if (!TxaExeSwitchUnSet('W'))           // unless explicitly unset,
      #else                                     // ALWAYS check on Windows :-)
         if (TxaExeSwitchSet('W'))              // or explicitly specified
      #endif                                    // default is 75 on startup
      {
         option = TxaExeSwitchNum( 'W', NULL, 0);
         if (option == 0)                       // -W or -W:0
         {
            //- slightly smaller than screen (on WIN, fixed for others :-)
            txwa->rsmaxlines = txwScrVisibleLines();
         }
         else
         {
            txwa->rsmaxlines = option + 2;      // correct for later adjust
         }
      }

      //- when in batch, always try the screen-resize (better progress info)
      if (TxScreenRows() > (USHORT) (txwa->rsmaxlines))
      {
         if ((option == 0) || TxaExeSwitch('b') || TxConfirm( 0,
            ...
            "Do you want to resize the window now ? [Y/N] : ", txwa->rsmaxlines))
         {
            TXLN       command;
            short      deltaY = TxScreenRows(); // current nr of rows
            short      deltaX = TxScreenCols(); // current nr of cols

            #if defined (WIN32)
               TRACES(("VisRows:%lu  VisCols:%lu\n", txwa->rsmaxlines, columns));

               if (txwa->rsmaxlines > 50)       // scale to 80% on large screens
               {
                  txwa->rsmaxlines = max( 51, (txwa->rsmaxlines * 4) / 5);
               }

               //- VisibleCols seems to be a constant 80! So, derive from Rows
               if      (txwa->rsmaxlines > 70)  columns = 120;
               else if (txwa->rsmaxlines > 60)  columns = 100;
               else if (txwa->rsmaxlines > 50)  columns =  90;
               else                             columns =  80;
            #endif

            sprintf( command, "mode %lu,%lu", columns, txwa->rsmaxlines -2);
            ...
            {
               deltaY -= TxScreenRows();
               window->border.bottom -= deltaY; // recalculate sizes
               window->client.bottom -= deltaY;

               deltaX -= TxScreenCols();        // delta could be negative!
               window->border.right  -= deltaX; // so window might get wider
               window->client.right  -= deltaX;
            }
         }
      }
   ...
   txwInitStandardDialogs();                    // attach help-texts and Init
   txwInitHexeditDialogs();                     // attach help-texts
   txwInitHelpDialogs();                        // attach help-texts
   ...
   #elif defined (DOS32)
   {
      ...
      TxxClearReg( regs);
      regs.w.bx = 0xffff;                       // avoid various bugs by
      regs.w.dx = 0xffff;                       // setting these non-zero
      regs.h.al = TXDX_DOS_GET_G_CP;            // GET active/system codepage
      TxxDosInt21( regs, TXDX_DOS_CODEPAGE);

      txwa->codepage = (ULONG) regs.w.bx;       // active codepage
   }
   #elif defined (WIN32)
      txwa->codepage = (ULONG) GetACP();
   #else                                        // UNIX, Linux/Darwin
      txwa->codepage = 0;                       // assume no 437 for now
   #endif
   TRACES(("codepage now: %lu\n", txwa->codepage));
...
)
{
   ...
   if ((wnd = txwValidateHandle( hwnd, &win)) != NULL)
   {
      BOOL     enabledNow = !(win->style & TXWS_DISABLED);

      TRACES(("enable: %lu <= %lu\n", enable, enabledNow));
      if (enable != enabledNow)                 // status change ?
      {
         if (enable)
         {
