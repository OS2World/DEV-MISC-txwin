...
#define TXWD_MB_WMIN        52                  // min window width messagebox
#define TXWD_MB_WDEFAULT    78                  // default window  width
...
#define TXWD_FD_WWIDTH      98                  // default window  width
...
typedef struct txwfdata                         // file dialog private data
{
   ...
   TX1K                lastValidPath;           // Last known valid path
...
static  char           txwNotReadyMsg[]  = "Not ready, disconnected LAN, unrecognized filesystem,\n"
                                           "or mistyped directory name. Revert to last valid ...";
...
#endif
                                 ;
static char txwfld_footer[]    = "F1=Help F2=wildcard F4=OK F12=Min Sort:Ctrl+X/E/N/F/T/B/U/R";
...
)
{
   ...
   strcpy( txwSaveAsPath, TxaExeSwitchStr( TXA_O_DIR, NULL, "")); // default to none (current DIR)
   strcpy( txwOpenFilePath,   txwSaveAsPath);
   strcpy( txwSelDirFilePath, txwSaveAsPath);
...
)
{
   ...
   if (txwIsWindow( TXHWND_DESKTOP))            // is there a desktop ?
   {
      ...
      if ((strlen( message) >= (TXWD_MB_WMIN - 2)) ||
          (strlen( title  ) >= (TXWD_MB_WMIN - 2)) ||
          (gwdata && (gwdata->count != 0)))
      {
      ...
      if (wvsize != 0)                          // do we have any widgets ?
      {
         if ((rc = txwCreateWidgets( mframe, gwdata, 1, 1)) == NO_ERROR)
         {
            ...
            txwCreateWindow( mframe, TXW_STLINE, mframe, 0, &window, txwDefWindowProc);
         }
      }
      ...
      window.tv.buf      = mText;
      mview = txwCreateWindow( mframe, TXW_TEXTVIEW, mframe, 0, &window, txwDefWindowProc);
      ...
      if (rbut1 != 0)                           // do we have a button 1 ?
      {
         ...
         mbut1 = txwCreateWindow(   mframe, TXW_BUTTON, mframe, 0, &window, txwDefWindowProc);
      ...
      if (rbut2 != 0)                           // do we have a button 2 ?
      {
         ...
         mbut2 = txwCreateWindow(   mframe, TXW_BUTTON, mframe, 0, &window, txwDefWindowProc);
      ...
      if (rbut3 != 0)                           // do we have a button 3 ?
      {
         ...
         mbut3 = txwCreateWindow(   mframe, TXW_BUTTON, mframe, 0, &window, txwDefWindowProc);
      ...
      if (rbut4 != 0)                           // do we have a button 4 ?
      {
         ...
         mbut4 = txwCreateWindow(   mframe, TXW_BUTTON, mframe, 0, &window, txwDefWindowProc);
...
)
{
   ...
   if (txwIsWindow( TXHWND_DESKTOP))            // is there a desktop ?
   {
      ...
      if (rc == NO_ERROR)
      {
         ...
         if (flags & TXPB_DECNUM_ONLY)
         {
            style |= TXES_DECNUM_ONLY;          // accept decimal digits only
         }
         else if (flags & TXPB_HEXNUM_ONLY)
         {
            style |= TXES_HEXNUM_ONLY;          // accept hexadecimal digits only
         }
         TRACES(("PromptBox entryfield style: 0x%8.8lx\n", style));
         txwSetupWindowData(
            ...
            eflength +2,                        // horizontal size  (incl [])
            style |                             // window frame style
         ...
         pentry = txwCreateWindow( pframe, TXW_ENTRYFIELD, pframe, 0, &window, txwDefWindowProc);
         ...
         pbtok = txwCreateWindow(   pframe, TXW_BUTTON, pframe, 0, &window, txwDefWindowProc);
         ...
         pbtcan = txwCreateWindow(  pframe, TXW_BUTTON, pframe, 0, &window, txwDefWindowProc);
...
)
{
   ...
   TRACES(("astatus:%8.8lx  miscInfo:%8.8lx\n",
       ...
       (list != NULL))                          // and do we have a list ?
   {
      ...
      ll = max( TxSelGetMaxLength( list), strlen( title)) +2;
      ...
      llist = txwCreateWindow( lframe, TXW_LISTBOX, lframe, 0, &window, txwDefWindowProc);
...
)
{
   ...
   if (txwIsWindow( TXHWND_DESKTOP))            // is there a desktop ?
   {
      ...
      int              uwidth = 0;              // total used menu width
      int              lwidth;                  // menu line length, this line
      ...
      short            decoration;              // decoration width at right
      TXS_MENU        *menu;
      ...
      if ((strlen(mbar->appDesc) && strlen(mbar->btnName)))
      {                                         // minimum decoration width
         decoration = 7;                        // closebutton + menu-cycle button
      }                                         // and optional version string
      else
      {
         decoration = 3;                         // closebutton only
      }
      ...
      for (m = 0, lwidth = 1; m < mbar->count; m++)
      {
         if ((menu = mbar->menu[m]) != NULL)
         {
            ...
            if ((lwidth + mwidth + decoration) > hsize)
            {
               if (lwidth > uwidth)             // for intermediate menu-line (wrap)
               {
                  uwidth = lwidth;              // remember longest menu line length
               }
            ...
            lwidth += mwidth;                   // add width of this menu item
            if (hsize > 96)
            {
               lwidth += 2;                     // more spacing on wide windows
            }
         }
      }
      if (lwidth > uwidth)                      // for last menu-line
      {
         uwidth = lwidth;                       // remember longest menu line length
      }
      if (uwidth + strlen( mbar->btnName) + 13 < hsize)
      {
         decoration = strlen( mbar->btnName) + 13; // recalculate nett decoration length
      }
      ...
      txwSetWindowUShort( bframe, TXQWS_X,  (USHORT) uwidth);
      txwSetWindowUShort( bframe, TXQWS_Y,  (USHORT) decoration);
      ...
      for (m = 0, line = 0, lwidth = 1; m < mbar->count; m++)
      {
         if ((menu = mbar->menu[m]) != NULL)
         {
            ...
            {
               lwidth = 1;
               line++;                          // move THIS one to next menu-line!
            }

            menu->list->miscInfo = (ULONG) mbar; // provide backward link
            style  = TXWS_A_MENU;
            ...
            bmenu = txwCreateWindow( bframe, TXW_LISTBOX, bframe, 0, &window, txwDefWindowProc);
   ...
   if (hwnd != 0)
   {
      ...
      switch (msg)
      {
         case TXWM_MENUSELECT:                  // pass to owner WinProc
            txwPostMsg( txwQueryWindow( hwnd, TXQW_OWNER), TXWM_COMMAND, mp1, TXCMDSRC_MENU);
         ...
         case TXWM_CHAR:
            switch (mp2)
            {
               ...
               default:
                  ...
                  else if (txwPossibleAsciiKey(mp2))
                  {
...
)
{
       ...
       (txwIsWindow( master))          )        // and is master valid ?
   {
      ...
      window.tv.markSize = 0;
      window.tv.buf      = text;
      tview = txwCreateWindow( tframe, TXW_TEXTVIEW, tframe, 0, &window, txwDefWindowProc);
...
)
{
   ...
   if (txwIsWindow( TXHWND_DESKTOP))            // is there a desktop ?
   {
      ...
      strcpy( fdpriv.lastValidPath, "");
//    strcpy( fdpriv.lastValidPath, fd->fPath); // could cause infinite loop if start path wrong!
      ...
      if (txfd_drivelist != NULL)
      {
         ...
         {
            window.dlgFocusID = TXFD_WID_ACCEPT;
         }
         ...
         if (rc == NO_ERROR)
         {
            ...
            fspinv = txwCreateWindow(  fframe, TXW_LISTBOX, fframe, 0, &window, txwSfdControlWinProc);
            ...
            if (fd->flags & TXFD_CREATE_NEW_DIR) // Create Dir button ?
            {
               ...
               fbut1 = txwCreateWindow(   fframe, TXW_BUTTON, fframe, 0, &window, txwSfdControlWinProc);
            ...
            txwSetupWindowData(
               ...
               &window);
            window.bu.text = txmb_bt_ok;
            fbtok = txwCreateWindow(   fframe, TXW_BUTTON, fframe, 0, &window, txwSfdControlWinProc);
            ...
            txwSetupWindowData(
               ...
               &window);
            window.bu.text = txmb_bt_cancel;
            fbtcan = txwCreateWindow(  fframe, TXW_BUTTON, fframe, 0, &window, txwSfdControlWinProc);
            ...
            fpath = txwCreateWindow(   fframe, TXW_STLINE, fframe, 0, &window, txwSfdControlWinProc);
            txwSetWindowUShort( fpath, TXQWS_ID, TXFD_WID_CFPATH);

            style = TXWS_LEFTJUSTIFY | TXWS_FRAMED | TXLS_FOOT_COUNT |
                    TXWS_HCHILD2SIZE               | TXWS_VCHILD_SIZE;
            txwSetupWindowData(
               wvsize +5, 0, position.bottom - 7 - wvsize, w2l,
               style, fd->basehelp,
               ' ', ' ',
            ...
            fdirec = txwCreateWindow( fframe, TXW_LISTBOX, fframe, 0, &window, txwSfdControlWinProc);
            ...
            strcpy( sort_files, "");
            strcpy( foot_count_files, "");
            ffname = txwCreateWindow( fframe, TXW_LISTBOX, fframe, 0, &window, txwSfdControlWinProc);
...
)
{
   ...
   if (hwnd != 0)
   {
      ...
      TXWFILEDLG   *fd     = txwQueryWindowPtr(    owner, TXQWP_USER);
      USHORT        target = 0;
      ...
      switch (msg)
      {
         case TXWM_CHAR:
            switch (mp2)
            {
               case TXk_DOWN:                   // additional control change
                  switch (wid)
                  {
                     case TXFD_WID_ACCEPT:
      ...
      switch (msg)
      {
         ...
         case TXWM_FD_NEWPATH:                  // new path, update others
            ...
            if ((strstr(        fd->fPath, TXFS_NOTREADY)   == NULL)  &&
                (TxTrueNameDir( fd->fPath, TRUE, spec) == NO_ERROR))
            {
               strcpy( fd->fPath, spec);        // the TrueName result
               ...
               {
                  strcpy( fdata->lastValidPath, fd->fPath);
               }
#ifndef UNIX
               //- Make driveletter for selected DIR CURRENT in the drive-list
               ...
               if ((lw = txwWindowData(  lhwnd)) != NULL)
               {
                  ...
                  lw->lb.list = list;
                  lw->lb.cpos = 0;
                  TRACES(("list vsize: %hu  selected: %lu  cpos:%lu\n", list->vsize, list->selected, lw->lb.cpos));
               }
            ...
            {
               TxMessage( TRUE, 0, "Inaccessible directory: '%s'\n\n%s", fd->fPath, txwNotReadyMsg);
               strcpy( fd->fPath, fdata->lastValidPath);
               txwPostMsg( hwnd, TXWM_FD_NEWPATH, 0, 0); // redo with last valid path
            }
            break;
            ...
            switch (mp2)                        // command source
            {
               case TXCMDSRC_PUSHBUTTON:
                  if (mp1 == TXFD_WID_CREATE)   // create directory
                  {
                     ...
                     if (rc != TXDID_CANCEL)
                     {
                        if ((dirname[0] != FS_PATH_SEP) && // not absolute from root
                            (dirname[1] != ':'        )  ) // or with a drive component
                        {
                           strcpy( newpath, fd->fPath);
                        }
                        #ifndef UNIX
                        else if (dirname[0] == FS_PATH_SEP) // absolute, no driveletter
                        {
                           TxCopy( newpath, fd->fPath, 3); // supply the letter and ':'
                        }
                        #endif
                        ...
                        {
                           sprintf( spec, "Error creating new directory: '%s'!", newpath);
                           TxMessage( TRUE, 0, spec);
                        }
                     }
                  }
                     ...
                     switch (TXSH1FROMMP(mp1))
                     {
                        ...
                        case TXFD_WID_FNAMES:
                           ...
                           {                    // path+filename returned
                              TxMessage( TRUE, 0,
                                         "Cannot select the higlighted filename!\n\n"
