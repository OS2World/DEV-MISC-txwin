...
//
// JvW 20170325: the 'mattr' and 'dmask' modify-attributes could be removed
//               from the FileTree() and RecursiveFindFile() functions.
//               They are NOT used by TxLib/DFSee and seem dangerous?
...
)
{
                    ...
                    TXS_AS_NOSTATIC, FALSE, NULL, &list) == NO_ERROR)
   {
      ...
      list->miscInfo = ffw;                     // input for FileDirAdd2List
      ...
      {
         txSelDestroy( &list);
         if ((strlen( fspec) <= 3) && (fspec[1] == ':')) // PC-style ROOT directory, no '..'
         {
            list = TxSelEmptyList( "ROOT, no subdirectories!", "ROOT-directory without subdirectories, nothing to select!", TRUE);
         }
         else                                   // possible empty subdirectory, assume '..' present
         {
      ...
      if (list != NULL)
      {
         TRACES(("Count:%4lu selected now: %lu = '%s'\n", list->count, list->selected,
   ...
   if (list != NULL)                            // do we have a list ?
   {
      ...
      if (rc == NO_ERROR)
      {
         if ((item = TxAlloc( 1, sizeof(TXS_ITEM))) != NULL)
         {
            ...
            //- note: filelist window width available in miscInfo lower 2 bytes
            ...
            if (list->miscInfo & TXFDS_ADD_DESCRIPTION)
            {
...
)
{
   ...
   memset( found, 0, sizeof(TXFILEFIND));       // initialize all fields
      ...
      if ((*handle = (TXHANDLE) FindFirstFile( fspec, &ffb)) != -1)
      {
         ...
         found->fSize = (((ULN64) ffb.nFileSizeHigh) << 32) + ffb.nFileSizeLow;
...
)
{
   ...
   strftime(text, TXMAXLN,  "%Y-%m-%d %H:%M:%S", gmtime( &(finfo->wTime)));
