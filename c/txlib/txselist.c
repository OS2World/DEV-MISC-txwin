...
)
{
   ...
   if (index < list->tsize)
   {
      if (list->markinfo != NULL)               // multiple select OK ?
      {
         BYTE  old = list->markinfo->markings[index];
         BYTE  new;
         ...
         if ((old == 0) && (new != 0) && (list->markinfo->marked < list->tsize))
         {
            list->markinfo->marked++;
         }
         if ((old != 0) && (new == 0) && (list->markinfo->marked != 0))
         {
            list->markinfo->marked--;
         }
         list->markinfo->markings[index] = new;
      }
...
)
{
   ...
   if (list->markinfo != NULL)                  // multiple mark OK ?
   {
      if (list->markinfo->marked != 0)
      {
         ULONG            i;

         for (i = 0; i < list->tsize; i++)      // visit all list entries
         {
            if ((list->markinfo->markings[i] & mask) == mask) // matches selection ?
            {
               if (list->vsize == list->tsize)  // all info available
               {
                  (markHandler)(list->items[i], &(list->markinfo->markings[i]), i);
               }
               else if (list->renderNewItem)
               {
                  (list->renderNewItem)(&item, &(list->markinfo->markings[i]), i);
                  (markHandler)(&item, NULL, i);
               }
               else
               {
                  (markHandler)( NULL, NULL, list->markinfo->marked);
               }
            }
         }
      }
   ...
   else                                         // callback for selected
   {
      if (list->vsize == list->tsize)           // all info available
      {
         (markHandler)(list->items[list->selected], NULL, list->selected);
      }
      else if (list->renderNewItem)
      {
         (list->renderNewItem)(&item, NULL, list->selected);
         (markHandler)(&item, NULL, list->selected);
      }
      else
      {
         (markHandler)( NULL, NULL, list->selected);
      }
   }
   RETURN (rc);
}                                               // end 'TxSelIterateSelected'
/*---------------------------------------------------------------------------*/
...
)
{
   ...
   if (list != NULL)
   {
      ...
      for (i = 0; i < list->count; i++)         // visit list entries
      {
         if ((item = list->items[i]) != NULL)   // existing item ?
         {
            if (list->items[i]->desc != NULL)   // has text string ?
            {
               if (strncasecmp( list->items[i]->desc, select, length) == 0)
               {
                  list->selected = i;           // set new selection
                  rc = i;
                  break;                        // end the search
...
)
{
   ...
   if ((rc = txSelCompValidateItems( i1, i2)) == 0)
   {
      ...
      TRACES(("%+3d i1:%P i2:%P  s1:%-30.30s  s2:%-30.30s\n",
...
// 20151005 JvW: Made sortinfo optional (needed for interactive stuff)
// 20160923 JvW: Preserve multiple-select info in markings by swapping it too
/*****************************************************************************/
...
)
{
   ...
   if (list)
   {
      for (iLo = 0; iLo < (list->count / 2); iLo++)
      {
         ...
         if (list->markinfo)                    // it is multiple-select
         {
            mark = list->markinfo->markings[ iLo];
            list->markinfo->markings[ iLo] = list->markinfo->markings[ iHi];
            list->markinfo->markings[ iHi] = mark;
         }
      }
      iLo = list->selected;
         ...
         if ((item = list->items[i]) != NULL)   // item to test
         {
            if (item->value == mid)             // matching item id ?
            {
               ...
               TRACES(("List %8.8lx item %d='%s' %sSET %s %s\n",
                        ...
                        (mask & TXSF_MARK_STD) ? "MARKED"   : ""));
               */
            }
            if ((item->flags & TXSF_P_LISTBOX) && // this is a popup/submenu
                (item->userdata != 0))          // and a list is attached.
            {
               TRACES(( "Recurse level %lu item:'%s' list: %8.8lx\n",
                         recurse +1, item->text, item->userdata));
...
)
{
   ...
   if ((selist = TxAlloc( 1, sizeof(TXSELIST))) != NULL)
   {
      ...
      if (multi && (tsize != 0))
      {
         TRACES(( "Allocate markinfo and array for %lu items\n", tsize));
         if ((selist->markinfo = TxAlloc( 1,  sizeof(TXSELMARK))) != NULL)
         {
            if ((selist->markinfo->markings = TxAlloc( tsize, sizeof(BYTE))) != NULL)
            {
               selist->astatus   |= TXS_AS_MARKINGS;
            }
            else
            {
               rc = TX_ALLOC_ERROR;
...
)
{
   ...
   if ((list != NULL) && ((*list) != NULL))
   {
      ...
      if ((as & TXS_AS_LST_DESC) && ((*list)->astatus & TXS_LST_DESC_PRESENT))
      {
         char         *descr = (char *) (*list)->miscInfo;

         TRACES(("Destroying list descr (miscInfo) at:%8.8lx\n", descr));
         TxFreeMem( descr);                     // avoid nasty cast warnings
         (*list)->miscInfo = 0;
      }
      if ((as & TXS_AS_MARKINGS) && ((*list)->markinfo != NULL))
      {
         TRACES(("Destroying markText and markinfo\n"));
         TxFreeMem( (*list)->markinfo->markings); // free markings array
         TxFreeMem( (*list)->markinfo);         // free marked count text
      }
...
)
{
   ...
   if ((list != NULL) && (list->count != 0))
   {
      ...
      if (list->markinfo != NULL)               // multi-select
      {
         if (list->markinfo->markings[ list->selected] != 0)
         {
            list->markinfo->marked--;           // update #marked count
         }
      }
      ...
      for (i = list->selected; i < list->count; i++)
      {
         ...
         list->markinfo->markings[i] = list->markinfo->markings[ i+1];
      }
...
)
{
   ...
   if (org != NULL)
   {
      ...
      marks = multi &&  org->markinfo && org->markinfo->markings;
      rc = TxSelCreate( org->vsize, org->tsize, items, 0, marks, NULL, list);
      ...
      if (rc == NO_ERROR)
      {
         ...
         if (share)
         {
            ...
            if (org->astatus & TXS_LST_DESC_PRESENT)
            {
               selist->miscInfo = org->miscInfo;
               selist->astatus |= TXS_LST_DESC_PRESENT;
            }
         }
         else
         {
            ...
            if (rc == NO_ERROR)
            {
               if (org->astatus & TXS_LST_DESC_PRESENT)
               {
                  char  *orgdescr = (char *) org->miscInfo;
                  char  *newdescr = TxAlloc( 1, strlen( orgdescr) + 1);

                  if (newdescr != NULL)
                  {
                     selist->astatus |= TXS_LST_DESC_PRESENT;
                     strcpy( newdescr, orgdescr);
                     selist->miscInfo = (ULONG) newdescr;
                  }
                  else
                  {
                     rc = TX_ALLOC_ERROR;
         ...
         if (rc == NO_ERROR)
         {
            if (marks)                          // keep same multi-selected
            {                                   // items selected
               memcpy( selist->markinfo->markings, org->markinfo->markings, org->tsize);
            }
         }
         else
         {
