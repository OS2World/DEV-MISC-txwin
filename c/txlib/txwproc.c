...
)
{
   ...
   ULONG               altcol = 0;
      ...
      static short     lastMoveRow = 0;      // vertical-move-only, for 'drag-select'
   #endif
   ...
   switch (msg)
   {
                  ...
                  case TXW_ARROW_SIZE:
                     if (win->style & TXWS_MOVEABLE) // move & resize allowed
                     {
                        ...
                        if (txwa->arrowMode == TXW_ARROW_MOVE)
                        {
                           ...
                           {
                              case TXk_UP:
                                 txwSetWindowPos( hwnd, 0,  0, -1, 0, 0, TXSWP_MOVE | TXSWP_RELATIVE);
                                 break;
                              ...
                              case TXk_LEFT:
                                 txwSetWindowPos( hwnd, 0, -1,  0, 0, 0, TXSWP_MOVE | TXSWP_RELATIVE);
                                 break;
                              ...
                              case TXk_PGUP:
                                 ...
                                 txwSetWindowPos( hwnd, 0,  0, delta, 0, 0, TXSWP_MOVE | TXSWP_RELATIVE);
                                 break;
                              ...
                              case TXk_HOME:
                                 ...
                                 txwSetWindowPos( hwnd, 0,  delta, 0, 0, 0, TXSWP_MOVE | TXSWP_RELATIVE);
                                 break;
                        ...
                        else                    // size window
                        {
                           ...
                           {
                              case TXk_UP:
                                 txwSetWindowPos( hwnd, 0,  0,  0,  0, -1, TXSWP_SIZE | TXSWP_RELATIVE);
                                 break;
                              ...
                              case TXk_LEFT:
                                 txwSetWindowPos( hwnd, 0,  0,  0,  -2, 0, TXSWP_SIZE | TXSWP_RELATIVE);
                                 break;
                              ...
                              case TXk_PGUP:
                                 ...
                                 txwSetWindowPos( hwnd, 0,  0,  0,  0, delta, TXSWP_SIZE | TXSWP_RELATIVE);
                                 break;
                              ...
                              case TXk_HOME:
                                 ...
                                 txwSetWindowPos( hwnd, 0,  0,  0,  delta, 0, TXSWP_SIZE | TXSWP_RELATIVE);
                                 break;
            ...
            switch (mp2)                        // key-value
            {
               ...
               case TXs_F5:                     // restore size/position
                  if (win->style & TXWS_MOVEABLE) // move & resize allowed
                  {
                     ...
                     if ((mp2 == TXa_F10) || (mp2 == TXs_F10)) // maximize to parent size
                     {
      ...
      case TXWM_STARTDRAG:                      // reset vertical-move-only filtering
         lastMoveRow = 0;
         break;
...
)
{
   ...
   else if (TXMOUSEKEYSCA() == TXm_KS_NONE)     // no shift/alt/ctrl keys
   {
      ...
      {
         //- to be refined, might present a context-menu (browser window?)
         //- Ignore for now since it is annoying when used with other mouse packages that are more useful :-)
      }
      ...
      else                                      // perform BUTTON-1 default actions
      {
         ...
         //- toggle the main menu on a click anywhere on desktop-border, menu-canvas
         //- top line of the scrollbuf (if no borders) or cmdline  (when menu is now up)
         else if (((hwnd == (TXWHANDLE) txwa->desktop) && (col > 1)) ||
                  ((hwnd == (TXWHANDLE) txwa->sbview ) && (row < 1)  && ((col + 3) < sx)) ||
                   ...
                   (hwnd == txwa->mainmenu))
         {
            ...
            if (txwa->mainmenu != 0)            // there IS a main menu
            {
                   ...
                   (TXMOUSECOL() < (win->client.right - 3   ))  ) // but BEFORE the (fake) [X]
               {
                  ...
                  txwPostMsg( txwQueryWindow( hwnd, TXQW_OWNER), TXWM_COMMAND, TXCMD_MENU_CYCLE, TXCMDSRC_MENU);
               }
            }
         }
