      ...
      switch (mp2)                              // key value
      {
         ...
         case TXc_O:
            if (dat->history != NULL)
            {
               ...
               {
                  sprintf( completion, "History (%lu)", list->count);
         ...
         case TXc_B:                            // delete to begin field
            //- strcpy( dat->buf, cur);         // undefined behaviour with overlap!
         ...
         default:
            ...
            {
               inRange = txwPossibleAsciiKey(mp2);
            }
            TRACES(("EF-style: 0x%8.8lx  mp2: 0x%8.8lx  inRange: %s\n", win->style, mp2, (inRange) ? "Yes" : "No"));
            if (inRange)
            {
            ...
            else if (win->style & TXLS_DROP_MENU)
            {
               ...
               switch (mp2)
               {
                  ...
                  case TXk_UP:
                     ...
                     {
                        txwPostMsg( owner, TXWM_MENUSELECT, mcode, TXMPFROM2SH(wid,0));
                     }
            ...
            else                                // multi-line
            {
               ...
               switch (mp2)
               {
                  ...
                  case TXa_RBRACKET:            // mark all 1st text-char NE uservalue
                     if (list->markinfo)        // in multiple mark lists
                     {
                        for (li = 0; li < list->count; li++)
                        {
                           ...
                           {
                              TxSelSetSelected( list, li, (mp2 == TXa_LBRACKET) ? TX_SET : TX_RESET, TXSF_MARK_STD);
                           }
                           else
                           {
                              TxSelSetSelected( list, li, (mp2 == TXa_LBRACKET) ? TX_RESET : TX_SET, TXSF_MARK_STD);
                           }
                        }
                  ...
                  case TXk_SPACE:               // + or SPACE, toggle mark
                     ...
                     {
                        break;                  // ignore keys on normal list (menu)
                     }
                  case TXk_DOWN:
                     do
                     {
                  ...
                  case TXc_S:  case TXc_T:  case TXc_U:  case TXc_X:  case TXc_Y:
