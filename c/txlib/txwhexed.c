...
// Author: J. van Wijk
//
// JvW  16-08-2017 TxLib 5.0; Allow very wide edit windows, more mouse suport
...
static char           *hexedhelp[] =
{
   ...
   "",
   "   Alt-T                      Toggle size between 16x32 and initial",
   "",
   ...
   "   Ctrl-Z"
#endif
            "                     Undo changes to CURRENT item (sector).",
   ...
   "",
   " Clicking on the left ASCII area '[' column, or in the absolute",
   " position area on the far right, will SCROLL the data up or down",
   " simulating a scrollbar. When clicking near the top or bottom of",
   " those, movement will be one line, when clicking near the center",
   " of the display, it will move by a whole page.",
#endif
   "",
...
)
{
   ...
   if (dat->curr->data != NULL)                 // is there any data at all?
   {
      if (dat->autosize)
      {
         ...
         TRACES(("Recalculated cols from sx: %hd to be: %hd\n", sx, dat->cols));
      }
      ...
      TRACES(("dat->pre3: %8.8lx dat->pre3->data: %8.8lx dat->pre3->size: %lu\n",
               dat->pre3,        dat->pre3->data,        dat->pre3->size));
      ...
      TRACES(("dat->nex3: %8.8lx dat->nex3->data: %8.8lx dat->nex3->size: %lu\n",
               dat->nex3,        dat->nex3->data,        dat->nex3->size));
      ...
      for (ln = 1, brow = 0; dat->curr && (brow < dat->rows); brow++, ln++)
      {
         ...
         for (bcol = 0; bcol < dat->cols; bcol++)
         {
                   ...
                   (offset + (LONG) dat->prev->size) < 0)
               {
                      ...
                      (offset + (LONG) dat->pre2->size + (LONG) dat->prev->size) < 0)
                  {
                     if (!dat->pre3  ||  !dat->pre3->data  ||
                         (offset + (LONG) dat->pre3->size + (LONG) dat->pre2->size + (LONG) dat->prev->size) < 0)
                     {
                        buffer = NULL;          // line is before PRE3 (draw --)
                     }
                     else
                     {
                        buffer  = dat->pre3->data; // line is in PRE3 buffer
            ...
            else
            {
               if (dat->nex2 && dat->nex2->data &&
                   (offset < dat->curr->size + dat->next->size + dat->nex2->size))
               {
               ...
               else
               {
                  if (dat->nex3 && dat->nex3->data &&
                      (offset < dat->curr->size + dat->next->size  + dat->nex2->size + dat->nex3->size))
                  {
                     buffer  = dat->nex3->data; // line is in NEX3 buffer
                  ...
                  {
                     buffer  = NULL;            // line is beyond NEX3 (draw --)
                  }
               }
            }
         ...
         if (buffer == dat->curr->data)         // line is in CURR buffer
         {
            txwStringAt( ln,      8,    line,         TxwSC( cHexEdHexByteStd));
         }
         else                                   // slightly different color
         {
            txwStringAt( ln,      8,    line,         TxwSC( cHexEdHexByteOff));
         }
         ...
         {
            txwStringAt( ln,  ac +1, ascii,        TxwSC( cHexEdAscByteOff));
         }
   ...
   {
      TRACES(("No data at all, skip painting ...\n"));
   }
   VRETURN();
}                                               // end 'txwPaintHexEdit'
/*---------------------------------------------------------------------------*/
...
)
{
   ...
   if ((he->rows > 0) && ((index < he->curr->size) || (he->curr->size == 0)))
   {
      ...
      if (show)                                 // setting new position
      {
         ...
         {
            sprintf( value, (he->decimal) ? "%6.6lu" : "%6.6lX", cursorpos);
            ...
            {
               sprintf( value, (he->decimal) ? "%13.13lu " : "0x%11.11lX ", absolute);
            ...
            if ((he->cols >= TXHE_MED_COLS) &&  // update Rsect value when enough room
                (he->curr->bps != 0))
            {
               sprintf( value, (he->decimal) ? "%2.2lu" : "%2.2lX", (cursorpos / he->curr->bps));
   ...
   if ((wnd != NULL) && ((win = wnd->window) != NULL))
   {
      ...
      switch (msg)
      {
         case TXWM_CREATE:                      // hexed control just created
            ...
            {
               txwPostMsg( hwnd, TXWM_CHAR, mp1, TXa_F10); // maximize to parent
            }
            if (dat->altView != 0)
            {
         ...
         case TXWM_CHAR:
            TRACES(( "key: %8.8lx\n", mp2));
            if ((dat->curr->size == 0) &&       // no data, empty. No edit actions!
                (mp2 != TXa_F2))                // except switching view
            {
               rc = txwDefWindowProc( hwnd, msg, mp1, mp2);
               break;                           // no movement or modify action
            }
            switch (mp2)                        // key value
            {
...
#if defined (UNIX)
               case TXa_Z:                      // c-Z caused program termination!
#else
               ...
               case TXa_PGUP:
                  ...
                  else if (dat->prev && dat->prev->data) // move to PREV, PRE2 or PRE3
                  {
                     if (modified)
                     {
                        wr = txwHexEditCallBack( TXHE_CB_WRITEBUF, dat);
                     ...
                     if (wr == NO_ERROR)
                     {
                        ...
                        else
                        {
                           ...
                           {
                              if (dat->pre3 && dat->pre3->data) // move to PRE3
                              {
                                 dat->posCurBuf += dat->prev->size;
                                 dat->posCurBuf += dat->pre2->size;
                                 dat->posCurBuf += dat->pre3->size;
                                 txwHexEditCallBack( TXHE_CB_PRE3_BUF, dat);
                              }
                              else
                              {
               ...
               case TXa_PGDN:
                  switch (mp2)
                  {
                     ...
                     case TXk_PGDN: if (wbytes < (dat->curr->size + dat->next->size))
                                    {
                                       value = wbytes;       break;
                                    }
                     default:       value = dat->curr->size; break;
                  ...
                  else if (dat->next && dat->next->data) // move to NEXT, NEX2 or NEX3
                  {
                     if (modified)
                     {
                        wr = txwHexEditCallBack( TXHE_CB_WRITEBUF, dat);
                     ...
                     if (wr == NO_ERROR)
                     {
                        ...
                        else
                        {
                           ...
                           {
                              if (dat->nex3 && dat->nex3->data) // move to NEX3
                              {
                                 dat->posCurBuf -= dat->curr->size;
                                 dat->posCurBuf -= dat->next->size;
                                 dat->posCurBuf -= dat->nex2->size;
                                 txwHexEditCallBack( TXHE_CB_NEX3_BUF, dat);
                              }
                              else
                              {
         ...
         case TXWM_BUTTONDBLCLK:
            if (txwMouseButtonDown( hwnd, msg, mp1, mp2) == TX_PENDING)
            {
               ...
               {
                  if ((col == ac)                 || // ON start ASCII area [
                      (col > (ac + dat->cols + 1) )) // BEYOND   ASCII area ] to end
                  {
                     ULONG   key = TXk_PGUP;
                  ...
                  else                          // move cursor left/right
                  {
                     txwPostMsg( hwnd, TXWM_CHAR, mp1, (col < 8) ? TXk_LEFT : TXk_RIGHT);
                  }
               }
