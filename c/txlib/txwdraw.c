...
{
   NULL,
   &txwcs_3g,                                   // TXWCS_3D_GREY             1
   &txwcs_3c,                                   // TXWCS_3D_CMDR             2
   &txwcs_st,                                   // TXWCS_STANDARD            3
   &txwcs_cm,                                   // TXWCS_COMMANDER           4
   &txwcs_nb,                                   // TXWCS_NO_BLINK            5
   &txwcs_3w,                                   // TXWCS_3D_WHITE            6
   &txwcs_bw                                    // TXWCS_BNW_ONLY            7
};
...
)
{
   ...
   if (rc == NO_ERROR)
   {
      ...
      if ((listdescr  = TxAlloc( 1, TXMAXTM)) != NULL)
      {
         ...
         selist->miscInfo = (ULONG) listdescr;
      }
...
/*****************************************************************************/
// Create selist from the available ANSI palette settings 1/2/8/16/88/256 Color
/*****************************************************************************/
...
)
{
   ULONG               rc    = NO_ERROR;        // function return
      ...
      ULONG            items = 6;
   #else
      ULONG            items = 3;               // just 1, 2, or 16 colors (PC)
   #endif
   ...
   if (rc == NO_ERROR)
   {
      ...
      if ((listdescr  = TxAlloc( 1, TXMAXTM)) != NULL)
      {
         sprintf( listdescr, "color palette to be used");

         selist->miscInfo = (ULONG) listdescr;
      }
      selist->selected = 1;                     // fail-safe default
      for (line = 0; (line < items) && (rc == NO_ERROR); line++)
      {
         TXS_ITEM  *item = TxAlloc( 1, sizeof(TXS_ITEM));
         ...
         if (item != NULL)
         {
                ...
                ((item->desc = TxAlloc( 1, TXMAXTM)) != NULL)  )
            {
               ...
               {
                  case 0:
                     item->value  = TXDID_MAX + 1;
                     strcpy( item->text, "  1 color B/W,  1 FG,  1 BG");
                     strcpy( item->desc, "Monochrome setting, for genuine old school terminals  ");
                     break;

                  case 1:
                     item->value  = TXDID_MAX + 2;
                     strcpy( item->text, "  2 color B/W,  2 FG,  1 BG");
                     strcpy( item->desc, "Monochrome setting, including intense/bright text     ");
                     break;

               #if defined (UNIX)

                  case 3:
                     item->value  = TXDID_MAX + 8;
                     strcpy( item->text, "  8 color std, 16 FG,  8 BG");
                     strcpy( item->desc, "Standard PC colors, but no bright/blinking backgrounds");
                     break;

                  case 4:
                     item->value  = TXDID_MAX + 88;
                     strcpy( item->text, " 88 color palette, 16 FG/BG");
                     strcpy( item->desc, "16 FG/BG colors from a 4-bits/color cube, total of  88");
                     break;

                  case 5:
                     item->value  = TXDID_MAX + 256;
                     strcpy( item->text, "256 color palette, 16 FG/BG");
                     strcpy( item->desc, "16 FG/BG colors from a 6-bits/color cube, total of 256");
                     break;
               #endif

                  default:
                     item->value  = TXDID_MAX + 16;
                     strcpy( item->text, " 16 color ext, 16 FG,  8 BG");
                     strcpy( item->desc, "Standard PC colors, including the bright backgrounds  ");
                     break;
               }
               if ((item->value - TXDID_MAX) == txwa->colors) // current one
               {
                  selist->selected = line;      // current palette
               }
               item->helpid = TXWH_USE_OWNER_HELP; // from owner-menu-item
            }
            else
   ...
   if ((txwValidateAndClip( hwnd, &win, TRUE, &clip)) != NULL)
   {
      ...
      if (style & TXWS_BORDERLINES)             // change to linestyle ...
      {
         switch (txwcs->linestyle)              // check scheme linestyle
         {
            ...
            default:          lc = (txwa->codepage == 437) ? txws_double : txws_dno437; break;
         }
         ...
         if (txwIsRectShowing( hwnd, &fill))    // whole titlebar visible ?
         {
            ...
            if (win->title && strlen(win->title))
            {
               ...
               if (((style & TXWS_LEFTJUSTIFY) || (strlen(win->title) > sx -2)) ||
                    (win->trhtext && strlen(win->trhtext)))
               {
                  pos = win->border.left;
                  if (style & TXWS_SIDEBORDERS)
                  {
            ...
            if (win->trhtext && strlen(win->trhtext))
            {
               TRACES(( "Window trhtext: '%s'\n", win->trhtext));
               pos = win->border.right - strlen(win->trhtext) -1;
               if ( (style & TXWS_BORDERLINES) && (pos > 1) &&
                   ((style & TXWS_TF_TEXTONLY) == 0)) // unless lines suppressed
               {
                  bc.ch = lc[TXLP_TXL];
                  ...
                  bc.ch = lc[TXLP_TXR];
                  txwScrDrawCellString( win->border.top,
                                        pos +strlen(win->trhtext),
                                        &clip, &bc, (short ) 1, TXSB_COLOR_NORMAL);
               }
               TRACES(("Write %u length trhtext to pos %hd\n", strlen(win->trhtext), pos));
               txwScrDrawCharStrCol( win->border.top, pos, &fill, win->trhtext, ((act) ?
                              TxwAC( cWinTitleFocus + csi_offset, win->titlefocus) :
                              TxwAC( cWinTitleStand + csi_offset, win->titlecolor)));
            }
      ...
      switch (win->class)
      {
         ...
         case TXW_CANVAS:
            ...
            {                                   // clear client window
               ...
               if (((TXWHANDLE) wnd) == txwa->mainmenu) // canvas for main menu
               {
                  ...
                  if (mbar && strlen(mbar->appDesc) && strlen(mbar->btnName)) // extra info available
                  {
                     short menulength = (short) txwQueryWindowUShort( hwnd, TXQWS_X);

                     if ((menulength + strlen(mbar->btnName) + 10) <= sx) // enough room for full button and version
                     {
                        if ((line = strchr( mbar->appDesc, '.')) != NULL)
                        {
                           sprintf( word, "[%s] %5.5s %s", mbar->btnName, line -2, TXW_CLOSEBUTTONSTRING);
                        }
                        else
                        {
                           sprintf( word, "[%s] %s", mbar->btnName, TXW_CLOSEBUTTONSTRING);
                        }
                        txwStringAt( 0, sx - strlen( word), word, at);
                        txwStringAt( 0, sx - strlen( word) + 1, mbar->btnName, TxwSC( cMbarHeadStand));
                     }
                     else                       // minimal usage, small buttons only (7 positions)
                     {
                        sprintf( word, "[m] %s", TXW_CLOSEBUTTONSTRING);
                        txwStringAt( 0, sx - 7, word, at);
                        strcpy( word, mbar->btnName);
                        word[1] = 0;
                        txwStringAt( 0, sx - 6, word, TxwSC( cMbarHeadStand));
                     }
                  }
         ...
         case TXW_LISTBOX:
            ...
            else                                // multi-line
            {
               ...
               {
                  //- Usually clientclear will be set to MenuTextStand for menus etc
               ...
               for (nr = 0; nr < sy; nr++)
               {
                  if ((list != NULL) && ((li = list->top + nr) < list->count))
                  {
                     ...
                     item = list->items[li];
                     itemflags = item->flags;     //- disabled, separator and color flags
                     if (list->markinfo == NULL)  //- marks in single lists, like menus
                     {
                        markflags = item->flags;
                     }
                     else                         //- marks in multiple-select lists
                     {
                        markflags = list->markinfo->markings[ li];
                     }
                     hasfocus = ((nr == win->lb.cpos) && (wnd == txwa->focus));
                     disabled = ((itemflags & TXSF_DISABLED ) != 0);

                     if (itemflags & TXSF_SEPARATOR)
                     {
                        strcpy(  string, " ");
                     ...
                     else
                     {
                        if (hasfocus)
                        {
                           ...
                           else if (itemflags & TXSF_TXTCOLOR1)
                           {
                              ctext = TxwAC( cListTxt1Focus    + csi_offset, win->lb.ccol);
                           }
                           else if (itemflags & TXSF_TXTCOLOR2)
                           {
                              ctext = TxwAC( cListTxt2Focus    + csi_offset, win->lb.ccol);
                           }
                           else if (itemflags & TXSF_TXTCOLOR3)
                           {
                              ctext = TxwAC( cListTxt3Focus    + csi_offset, win->lb.ccol);
                           }
                           else
                           {
                        ...
                        else
                        {
                           ...
                           {
                              ctext = TxwSC( cListTxt1Stand    + csi_offset);
                           }
                           else if (itemflags & TXSF_TXTCOLOR2)
                           {
                              ctext = TxwSC( cListTxt2Stand    + csi_offset);
                           }
                           else if (itemflags & TXSF_TXTCOLOR3)
                           {
                              ctext = TxwSC( cListTxt3Stand    + csi_offset);
                           }
                           else
                           {
                        ...
                        if      (markflags & TXSF_MARK_STD )
                        {
                           mark[0] = (txwa->codepage == 437) ? 'û' : '+';
                           if (list->markinfo != NULL) // multiple mark (BROWSE)
                           {
                              ctext = cmark;
                           }
                        }
                        ...
                        TRACES(("csi_offset = %d  ctext:%2.2hhx  cselc:%2.2hhx  cmark:%2.2hhx\n",
                                 csi_offset, ctext, cselc, cmark));

                        txwStringAt( nr, 0, mark, cmark);

                        strcpy(  string, list->items[ li]->text);
                        TxPClip( string, sx -1, ' ');
                        if (hasfocus)           // need to make it stand-out WITHOUT using color?
                        {
                           if ((txwa->colors < 8) || // need extra visual feedback
               ...
               if (win->style & TXLS_FOOT_COUNT) // footer must be a TXTT minimum
               {
                  if (list != NULL)
                  {
