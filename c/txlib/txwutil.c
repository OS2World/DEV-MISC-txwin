...
)
{
   switch (msg)
   {
      ...
      case TXWM_STARTDRAG:       return( "WM_STARTDRAG");
      ...
      case TXWM_SCREENRESIZE:    return( "WM_SCREENRESIZE");
      default:
         if (msg < TXWM_USER)
         {
...
)
{
   switch (key)
   {
      ...
      case TXs_INSERT:             return( "s_INSERT");
      case TXs_DELETE:             return( "s_DELETE");
      case TXs_UP:                 return( "s_UP");
      case TXs_DOWN:               return( "s_DOWN");
      case TXs_LEFT:               return( "s_LEFT");
      case TXscLEFT:               return( "scLEFT");
      case TXs_RIGHT:              return( "s_RIGHT");
      case TXscRIGHT:              return( "scRIGHT");
      case TXs_PGUP:               return( "s_PGUP");
      case TXs_PGDN:               return( "s_PGDN");
      case TXs_HOME:               return( "s_HOME");
      case TXs_END:                return( "s_END");
...
)
{
   ...
   if (wnd && ((win = wnd->window) != NULL) && (win->class == TXW_ENTRYFIELD))
   {
      ...
      if (dat->history != NULL)
      {
         if (txwa->historyMode & TXW_HIST_UNIQUE) // don't add duplicates
         {
...
// 20151005 JvW: Create list in reversed order, latest entry LAST, not first
/*****************************************************************************/
...
)
{
   ...
   if (hb != NULL)
   {
      ...
      {
         TRACES(( "Matching:'%s', count:%2d order:%8.8lx string:'%s'\n",
      ...
      if (items != 0)
      {
         ...
         if (rc == NO_ERROR)
         {
            ...
            ULONG            line;              // index in the list (subset)
            ...
            if (rc == NO_ERROR)
            {
               //- reverse sorting order (latest entry at END of the list)
...
{
   sprintf( field, (hexadec) ? "%*llX" : "%*llu" , length, value);
   if (hwnd)
   {
      txwInvalidateWindow( hwnd, FALSE, FALSE);
   }
...
)
{
   ...
   TxStrip( temp, field, ' ', ' ');
   sscanf(  temp, (hexadec) ? "%llX" : "%llu", value);
}                                               // end 'txwDlgField2U64'
/*---------------------------------------------------------------------------*/
...
)
{
   ...
   if (((win = txwWindowData( hwnd)) != NULL) && (win->class == TXW_LISTBOX))
   {
      ...
      if (list && list->sortinfo)
      {
         ...
         if (strlen( win->frhtext) > 3)
         {
            if (win->frhtext[ strlen(win->frhtext) - 1] == '>')
            {
               win->frhtext[ strlen(win->frhtext) - 2] = (descend) ? 'D' : 'A';
            }
         }
...
)
{
   ...
   if (((win = txwWindowData( hwnd)) != NULL) && (win->class == TXW_LISTBOX))
   {
      ...
      TRACES(("list:%8.8lx  markinfo:%8.8lx\n", list, list->markinfo));
      if (list && list->markinfo)
      {
         win->trhtext = list->markinfo->markText; // point to available buffer
         if (list->markinfo->marked != 0)
         {
            sprintf( win->trhtext, "Marked:%6lu", list->markinfo->marked);
         }
         else
         {
            strcpy(  win->trhtext, "");         // empty when count ZERO
         }
         TRACES(("TRH text:'%s'\n", win->frhtext));
      }
   }
   VRETURN();
}                                               // end 'txwListMark2trhText'
/*---------------------------------------------------------------------------*/
