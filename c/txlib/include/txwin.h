...
#define TXW_MAX_COLS      768                   // absolute maximum for screen width
...
typedef enum txwmsgid                           // TX window messages
{
   ...
   TXWM_BUTTONDBLCLK,                           // mouse dblclk (quick UP->DOWN)
   TXWM_STARTDRAG,                              // start drag (next will be MOUSEMOVE)
   TXWM_MOUSEMOVE,                              // mouse movement  (window drag)
   ...
   TXWM_SCREENRESIZE   = 999,                   // Notify screen was resized (re-init!)
...
#define TXm_WHEELUP    0x0008                   // Wheel moved UP
#define TXm_WHEELDN    0x0010                   // Wheel moved DOWN
...
typedef struct txentryfield                     // entry field info
{
   ...
   short               rsize;                   // real allocated size (str limit)
...
typedef enum txhexedcmd
{
   ...
   TXHE_CB_NEX3_BUF,                            // make 'nex3' buffer current
   ...
   TXHE_CB_PRE3_BUF,                            // make 'pre3' buffer current
...
typedef struct txhebuf                          // hex editor data buffer
{
   ...
   ULN64               start;                   // abs start position (ref)
   ULONG               size;                    // buffersize, bytes (item)
...
typedef struct txhexedit                        // hex editor control
{
   ...
   USHORT              rows;                    // nr of rows shown (screen)
   USHORT              cols;                    // nr of bytes per row (var)
   USHORT              initialRows;             // nr of rows, set at startup
   USHORT              initialCols;             // nr of bytes set at startup
   BOOL                autosize;                // set 8/16/32/64/128 columns
   BOOL                ascii;                   // cursor in ascii area
   BOOL                decimal;                 // use decimal positions
   TXHEBUF            *pre3;                    // buffer BEFORE prev   prev
   ...
   TXHEBUF            *nex3;                    // buffer AFTER  next   next
   ...
   ULN64               markBase;                // valid curr->start for mark
   void               *userdata;                // application user data
   void               *actionCallback;          // application action handler
   ULONG               wFlags;                  // Hexed window control flags
   USHORT              altView;                 // start alternate, if not 0
...
//- specific widget macro's that include a Window-procedure
#define TXWprEntryf(v,s,proc,text) text, TXW_ENTRYFIELD, 0, proc, TXWgEntryfield(0,0,0,(s),0,0,NULL,(v))
#define TXWprStatic(    proc,text) "",   TXW_STATIC,     0, proc, TXWgStatic(text)
#define TXWprStline(    proc,text) "",   TXW_STLINE,     0, proc, TXWgStline(text)
#define TXWprButton(var,proc,text) "",   TXW_BUTTON,     0, proc, TXWgButton(var,text)
#define TXWprLstBox(v,p,proc,text) text, TXW_LISTBOX,    0, proc, TXWgListbox((p),0,0,0,0,0,0,0,0,(v))
...
#define TXES_DECNUM_ONLY      0x20000000        // allow decimal numbers only
#define TXES_HEXNUM_ONLY      0x40000000        // allow hexadecimal numbers only
...
#define TXLS_NEEDDBLCLK       0x10000000        // need DBLCLK to ENTER (filelist)
...
#define TXWS_ENTRYHEX TXWS_ENTRYBT     | TXES_HEXNUM_ONLY
#define TXWS_ENTRYDEC TXWS_ENTRYBT     | TXES_DECNUM_ONLY
...
// Pre-defined / special values for TXWM_COMMAND id values
#define TXCMD_MENU_CYCLE         0xface2ace
...
#define TXPB_DECNUM_ONLY             0x0100
#define TXPB_HEXNUM_ONLY             0x0200
...
// Hex edit standard dialog flags
#define TXHE_MOVEABLE                0x2000
#define TXHE_MAXIMIZE                0x4000     // maximize on startup
...
#define TXWCS_3D_GREY             1
#define TXWCS_3D_CMDR             2
#define TXWCS_STANDARD            3
#define TXWCS_COMMANDER           4
#define TXWCS_NO_BLINK            5
#define TXWCS_3D_WHITE            6
#define TXWCS_BNW_ONLY            7
#define TXWCS_LAST_SCHEME         7
...
#define TXWSCHEME_ERRMSG cWinClientErr
...
typedef enum txw_wincolors                      // color scheme entity indexes
{
   ...
   cWinClientErr,                               // Client error color
   cWinBorder_top,                              // top
   ...
   cListTextFocus,                              // ccol
   cListTxt1Stand,                              // alt1 text
   cListTxt1Focus,                              // alt1 text focus
   cListTxt2Stand,                              // alt2 text
   cListTxt2Focus,                              // alt2 text focus
   cListTxt3Stand,                              // alt3 text
   cListTxt3Focus,                              // alt3 text focus
   ...
   cFileTextStand,                              // text
   cFileTextFocus,
   cFileTxt1Stand,                              // alt1 text
   cFileTxt1Focus,                              // alt1 text focus
   cFileTxt2Stand,                              // alt2 text
   cFileTxt2Focus,                              // alt2 text focus
   cFileTxt3Stand,                              // alt3 text
   cFileTxt3Focus,                              // alt3 text focus
   ...
   cMenuTextStand,                              // text
   cMenuTextFocus,
   cMenuTxt1Stand,                              // alt1 text
   cMenuTxt1Focus,                              // alt1 text focus
   cMenuTxt2Stand,                              // alt2 text
   cMenuTxt2Focus,                              // alt2 text focus
   cMenuTxt3Stand,                              // alt3 text
   cMenuTxt3Focus,                              // alt3 text focus
   ...
   cHexEdItemHlight,
   cHexEdModifyText,
   cHexEdHexByteOff,                            // located outside active buffer
   cHexEdAscByteOff,
   cNrOfSchemeColors
...
#define  TX_Blue_on_Brown      (TXwCnY + TXwCNB)
...
#define  TX_Magenta_on_Grey    (TXwCbZ + TXwCNM)
...
#define  TX_Green_on_Grey      (TXwCbZ + TXwCNG)
...
// Keys that should be treated as normal ascii input characters (rest to owner)
#define  txwPossibleAsciiKey(k)   (((k) >= TXk_SPACE) && ((k) < TXW_KEY_GROUP_1) && ((k) != TXk_BACKQUOTE))
...
// numpad functions when NO numlock, on OSX+Karabiner Fn+number
#define TXk_INSERT              0x152           // numpad 0
#define TXk_DELETE              0x153           // numpad .
#define TXk_UP                  0x148           // numpad 2
#define TXk_DOWN                0x150           // numpad 8
#define TXk_LEFT                0x14b           // numpad 4
#define TXk_NUM_5               0x14c           // numpad 5
#define TXk_RIGHT               0x14d           // numpad 6
#define TXk_PGUP                0x149           // numpad 9
#define TXk_PGDN                0x151           // numpad 3
#define TXk_HOME                0x147           // numpad 1
#define TXk_END                 0x14f           // numpad 7
...
//- not generated by PC, but some/most ARE there on UNIX (values a_xxx + 0x20)
#define TXs_INSERT              0x1c2
#define TXs_DELETE              0x1c3
#define TXs_UP                  0x1b8
#define TXs_DOWN                0x1c0
#define TXs_LEFT                0x1bb
#define TXs_RIGHT               0x1bd
#define TXs_PGUP                0x1b9
#define TXs_PGDN                0x1c1
#define TXs_HOME                0x1b7
#define TXs_END                 0x1bf

//- Shift+Ctrl may be generated as unique keys on some UNIX terminals (LXTerminal?)
#define TXscLEFT                0x1cb
#define TXscRIGHT               0x1cd
...
#define TXc_K                   0x00b
#define TXc_L                   0x00c           // TxLIB reserved (refresh)
...
typedef struct txw_input_event
{
   ...
   ULN64               tmr;                     // timestamp  (DBLCLK detect)
...
// Flush screen caches and invalidate all windows, causing complete repaint
void txwRefreshScreen
(
   void
);
...
#define TXWMAX_PALETTE 256
// Create selist from the available ANSI palette settings 2/8/16/88/256 Color
ULONG TxSelistColorPalettes                     // RET   result
(
   TXSELIST          **list                     // OUT   selection list
);
...
// Update entryfield from USHORT integer value to HEX/DEC string form
void txwDlgUs2Field
(
...
// Update entryfield from ULONG integer value to HEX/DEC string form
void txwDlgUl2Field
(
   TXWHANDLE           hwnd,                    // IN    field handle
...
// Update entryfield from ULN64 integer value to HEX/DEC string form
void txwDlgU642Field
(
   TXWHANDLE           hwnd,                    // IN    field handle
   ULN64               value,                   // IN    integer value
   char               *field,                   // INOUT field value
   int                 length,                  // IN    formatting length
   BOOL                hexadec                  // IN    hexadecimal formatted
);

// Update USHORT integer value from entryfield HEX/DEC string form
void txwDlgField2Us
(
...
// Update ULN64 integer value from entryfield HEX/DEC string form
void txwDlgField2U64
(
   ULN64              *value,                   // OUT   integer value
   char               *field,                   // IN    field value
   BOOL                hexadec                  // IN    hexadecimal formatted
);
...
// Update title right-hand text based on current list #marked items
void txwListMark2trhText
(
   TXWHANDLE           hwnd                     // IN    window with attached list
);
...
// Set INSERT mode on/off, return current setting of the keyboard INSERT mode
BOOL txwSetInsertMode                           // RET   insert mode is ON
(
   BOOL                newMode                  // IN    insert mode to set
);
