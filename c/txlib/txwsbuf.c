...
// JvW  10-08-2017 Make sure SbView does never paints beyond its borders

#include <txlib.h>                              // public interface
#include <txwpriv.h>                            // private window interface


#define TXWP_ARG_SIZE   6                       // max digits in arguments
#define TXWP_NUM_ARGS  10                       // max number of arguments
...
)
{
   ...
   {
      TRACES(("width: %lu  length: %lu  vsize: %lu\n", sbuf->width, sbuf->length, sbuf->vsize));

      if (sbuf->vsize <= sbuf->length)
      {
...
)
{
   ...
   while (*current)
   {
      ...
      {
         if (*current != 0x0d)                  // ignore remaining CR from CR/LF (Unix)
         {
...
/*****************************************************************************/
// Parse known ANSI clause including arguments
// Unrecognized sequences will end-up 'as-is' in the scrollbuffer!
/*****************************************************************************/
static char *txwpParseAnsiArgs
(
   char               *current,                 // IN    this character
   ...
   else
   {
      ...
      else
      {
         switch (*current)
         {
            ...
            case 'm':
               if ((argument[0] == 48) && (argument[1] == 5) && // BG+FG color from a 88/256 palette
                   (argument[3] == 38) && (argument[4] == 5) && (count == 6))
               {
                  TXTT sequence;                // recreated ANSI escape sequence for this color

                  sprintf( sequence, "[48;5;%d;38;5;%dm", argument[2], argument[5]);


                  if (TxReverseLookupAnsi( sequence, &txwpBgColor, &txwpFgColor) == FALSE)
                  {
                     *syntax = TRUE;            // will echo the actual escape-sequence to screen
                  }
                  //- fprintf( stderr,  "ANSI lookup: %-24s %s  bg:%x  fg:%x\n",
                  //-          sequence, (*syntax) ? "NOT found" : "found OK!", (int) txwpBgColor, (int) txwpFgColor);
               }
               else                             // other ANSI color/attribute strings
               {
                  for (i = 0; i < count; i++)
                  {
                     switch (argument[i])
                     {
                        case  0 :
                           ...
                           {
                              if (txwa->colors == 4) // white/yellow on Blue output window
                              {
                                 txwpFgColor = TXwCBW; // default foreground color, bright white
                                 txwpBgColor = TXwCnB; // default background color, blue
                              }
                              else
                              {
                              }
                              txwpFgColor = TXwCNW; // default foreground color, standard white
                              txwpBgColor = TXwCnZ; // default background color, black
                           }
                           else
                           {
                              intensFg = 0;
                        ...
                        case 1:                 // High intensity (FG)
                           if (count == 1)      // Just INTENSE (no color)
                           {
                              if (txwa->colors == 4) // white/yellow on Blue output window
                              {
                                 txwpFgColor = TXwCBY; // default foreground color, bright yellow
                                 txwpBgColor = TXwCnB; // default background color, blue
                              }
                              else
                              {
      ...
      switch (msg)
      {
         case TXWM_CHAR:
            switch (mp2)                        // key value
            {
               ...
               case TXs_UP:
               case TXa_LEFT:
                  if (dat->topline >  dat->sbdata->firstline)
                  {
                     dat->topline--;
                     upd = TRUE;
                  }
               ...
               case TXa_PGUP:
               case TXs_PGUP:
                  if (dat->topline > (ULONG) sy + dat->sbdata->firstline)
                  {
                     dat->topline -= sy;
                     upd = TRUE;
                  }
                  else if (dat->topline != dat->sbdata->firstline)
                  {
