...
)                                               //       or NULL to query
{
   ...
   if ((opt >= TXA_LBASE) && ((lni = (opt - TXA_LBASE)) < TXA_LSIZE))
   {
      ...
      rc = &(txalongopt[lni][0]);
      TRACES(("Name for APP option %lu set to '%s'\n", opt - TXA_O_APP0, rc));
   }
   RETURN (rc);
...
)                                               //       not needed use NULL
{
   ...
   TRLEVX(700,( "TxaParseNumber: value: '%s'  rclass: 0x%lX\n", value, rclass));
   ...
   if (unit != NULL)                            // read unit char suffix
   {
      ...
      TRLEVX(700,( "TxaParseNumber: Unit for value: '%s' is: '%c'\n", value, *unit));
   }
   if (*value == '0')                           // possible hex/oct/dec prefix
   {
      switch (tolower(*(value +1)))
      {
         case 'x': sscanf( value +2, "%llx", &rc); break; // hexadecimal number
         case 'o': sscanf( value +2, "%llo", &rc); break; // octal number base
         case 'n':
         case 't': sscanf( value +2, "%llu", &rc); break; // decimal number base
   ...
   if (prefix == FALSE)                         // no prefix, test HEX or DEC
   {
      ...
      {
         TRLEVX(700,( "TxaParseNumber: auto-interpret as HEX!\n"));
         sscanf( value, "%llx", &rc);           // interpret as HEX
      }
      else                                      // could be both ...
      {
         if (txwa->radixclass & radixclass)     // class set to HEX default
         {
            TRLEVX(700,( "TxaParseNumber: Radix-selection HEX!\n"));
            sscanf( value, "%llx", &rc);        // interpret as HEX
         }
         else
         {
            TRLEVX(700,( "TxaParseNumber: Radix-selection DEC!\n"));
            sscanf( value, "%llu", &rc);        // interpret as DECimal
         }
      }
   }
   else
   {
      TRLEVX(700,( "TxaParseNumber: Selection by prefix '%2.2s'\n", value));
   }
   return (rc);
}                                               // end 'TxaParseNumber'
/*---------------------------------------------------------------------------*/
         ...
         switch (*value)
         {
            ...
            case '5': case '6': case '7': case '8': case '9':
               txa->opt[ index].value.number =
                  TxaParseNumber( value, TX_RADIX_STD_CLASS, &(txa->opt[ index].unit));
...
)
{
   ...
   if ((txa = txaHandle2Element( txh)) != NULL)
   {
      if (((BYTE) opt >= TXA_BASE) && ((BYTE) opt < (TXA_BASE + TXA_SIZE)))
      {
         ...
         TRARGS(("TXH (%s): %lx option: %3hu = '%c'  value: %llu '%s'\n",
...
)
{
   ...
   ENTER();
   TRACES(("Option: '%c' default: %llx mod:%2.2hx = '%c'\n",
            option, def, mod, mod));
